...
export const queryKeys = {
  ...
  songSuggestions: (id: string, limit: number) => ['song-suggestions', id, limit] as const,
  artist: (id: string, page: number, songCount: number, albumCount: number, sortBy: string, sortOrder: string) =>
...
// Core entity hooks
export function useAlbum(id: string, options?: Omit<UseQueryOptions<DetailedAlbum>, 'queryKey' | 'queryFn'>) {
  return useQuery({
...
export function usePlaylist(id: string, options?: Omit<UseQueryOptions<DetailedPlaylist>, 'queryKey' | 'queryFn'>) {
  return useQuery({
...
export function useSong(id: string, options?: Omit<UseQueryOptions<DetailedSong[]>, 'queryKey' | 'queryFn'>) {
  return useQuery({
...
export function useArtist(
  ...
  options?: {
    ...
    sortOrder?: 'asc' | 'desc'
    queryOptions?: Omit<UseQueryOptions<DetailedArtist>, 'queryKey' | 'queryFn'>
  }
) {
  const { page = 1, songCount = 10, albumCount = 10, sortBy = 'popularity', sortOrder = 'desc', queryOptions } = options || {}
  
  return useQuery({
    queryKey: queryKeys.artist(id, page, songCount, albumCount, sortBy, sortOrder),
    queryFn: async () => {
      const response = await getArtistById(id, { page, songCount, albumCount, sortBy, sortOrder })
      return response.data
    },
...
export function useArtistSongs(
  ...
  options?: Omit<
    UseInfiniteQueryOptions<{ total: number; songs: DetailedSong[] }, Error, { total: number; songs: DetailedSong[] }, readonly [string, string, string, string], number>,
  ...
  return useInfiniteQuery({
    ...
    getNextPageParam: (lastPage, allPages) => {
      const currentTotal = allPages.reduce((sum, page) => sum + page.songs.length, 0)
...
export function useArtistAlbums(
  ...
  sortOrder: 'asc' | 'desc' = 'desc',
  options?: Omit<
    UseInfiniteQueryOptions<{ total: number; albums: DetailedAlbum[] }, Error, { total: number; albums: DetailedAlbum[] }, readonly [string, string, string, string], number>,
    'queryKey' | 'queryFn' | 'getNextPageParam' | 'initialPageParam'
  >
) {
  return useInfiniteQuery({
    ...
    getNextPageParam: (lastPage, allPages) => {
      const currentTotal = allPages.reduce((sum, page) => sum + page.albums.length, 0)
      return currentTotal < lastPage.total ? allPages.length + 1 : undefined
    },
    initialPageParam: 0,
    staleTime: 1000 * 60 * 5, // 5 minutes
...
export function useSearchSongs(
  ...
  limit = 20,
  options?: Omit<
    UseInfiniteQueryOptions<{ total: number; results: DetailedSong[] }, Error, { total: number; results: DetailedSong[] }, readonly [string, string], number>,
    'queryKey' | 'queryFn' | 'getNextPageParam' | 'initialPageParam'
  >
) {
  return useInfiniteQuery({
    ...
    getNextPageParam: (lastPage, allPages) => {
      const currentTotal = allPages.reduce((sum, page) => sum + page.results.length, 0)
      return currentTotal < lastPage.total ? allPages.length + 1 : undefined
    },
    initialPageParam: 0,
